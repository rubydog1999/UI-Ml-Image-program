...
def doDescription(value):
    ...
    if value == "K-mean":
        ...
        myDes = Label(description_layer, text="Image Segmentation involves converting an image into a collection of regions \n of pixels"
...
def run():
    ...
    if values == "K-mean":
        ...
        def k_means(img, k, max_iter):
            ...
            return clusters
        print()
        cl = k_means(img=imgRGB, k=k_val, max_iter=10)
        table_element_2 = []
        for j in cl:
            for z in cl[j]:
...
subMenu.add_command(label="Open Image", command=openimg)
